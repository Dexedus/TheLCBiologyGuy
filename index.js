...
async function sendDailyEmailReport() {
  ...
  try {
    ...
    // Get emails and first names added in the last 24 hours from testname table
    const testnameResult = await db.query(`
      SELECT email, first_name FROM testname
      WHERE created_at >= NOW() - INTERVAL '1 day'
    `);

    // Decrypt emails and format with first name for testname table
    let testnameEmailList = testnameResult.rows.map(row => {
      const decryptedEmail = decrypt(row.email);
      const first_name = row.first_name || 'Valued User';
      return `<li>${decryptedEmail} (${first_name})</li>`;
    });
    if (testnameEmailList.length === 0) {
      testnameEmailList = ['<li>No new entries over the last 24 hours</li>'];
    }
    ...
    const msg = {
      ...
      html: `
        <p>New emails added to the free resources list in the last 24 hours:</p>
        <ul>${emailList.join('')}</ul>
        <p>Emails added to the waiting list in the last 24 hours:</p>
        <ul>${testnameEmailList.join('')}</ul>
      `
    };
      ...
      lineItems.data.forEach(item => {
        ...
        if (products[0] === 'Cell & Ecology Free Masterclasses (March 6th and 9th)') {
          ...
          productTable = 'testtable';
            ...
            message = `Hey ${firstName},<br><br>The Halloween Masterclasses will run on the 27th and 28th of October.<br><br>Classes will start at 10am on both days.<br><br>Both classes will be recorded and uploaded to this Google Drive: https://u48917275.ct.sendgrid.net/ls/click?upn=u001.gb1oIQZYL4vnMZkgmvEgigzFl42rVVPLGu-2Fe519Dvun9tuRbO-2FbM7IplLEtFJNpQ05TKwRq03odmolpArth0ldjiurLFB4dCM-2B4tixT-2F0TJ1ELxqIhhbS32gO3hKFnrEIFcd_4pE3C559McDKAd-2Fg3v7vn7eIndNn6ci9X9Lg05SN5hd0HqQd0CGpTiKRONJude4-2BSsNEXmpTWFbVn7KIYUZRVHAyrUpW7MXxjc-2FqCDWugVFXx574jVw6J7AuqIMN8xCK0iv3bPZjXrabb-2BWXwezZpQFLZE34yn6CVbJCQvmrQ3rjg5a43SNZwK-2BgAipFyVeR3EkkRmw-2B21-2FGCOBGcKlZTw-3D-3D<br><br>You will join Day 1 (Cell Biology) using this link:<br>https://us06web.zoom.us/j/86720349492?pwd=XJWfPS8zXbkKsnRV4SaZptAvgvAUY5.1<br>Passcode: 475432<br><br><br>Day 2 (Ecology) using this link:<br>https://us06web.zoom.us/j/82704111200?pwd=iA0kyBba0sl0JNiNBblSpPRglbalrx.1<br>Passcode: 022398<br><br>Best,<br>Max<br><br><b>If you'd like to opt-out of future promotional emails, please click <a href="https://www.thelcbiologyguy.ie/#opt" target="_blank">Here</a> and submit your email address. Thanks!<b/>`;
...
app.get('/waiting', (req, res) => {
    ...
    res.render("waitingList.ejs", { success: success });
})
...
app.post('/waiting', async (req, res) => {
    try {
        ...
        if (result.rows.length === 0) {
            ...
            console.log(`Added ${firstName} (${email}) to testname database table`);
        } else {
            console.log('Email already exists in testname table');
        }
        
        res.redirect('/waiting?success=true');
    } catch (error) {
